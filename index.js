...
function getYears(array, getFinalsCB) {
  const yearArray = array.map((item) => item.Year);
  getFinalsCB(yearArray);
  return yearArray;
}
...
function getWinnersByYear(array, getFinalsCB, getYearsCB, getWinnersCB) {
  ...
  const years = getYearsCB(array, getYearsCB);
  return winners.map(
...
function getAverageGoals(array) {
  const averageHomeGoals = array.reduce(
    (acc, item) => acc + item["Home Team Goals"] + item["Away Team Goals"],
    0
  );
